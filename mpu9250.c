	...
	{
		HAL_I2C_Mem_Write(&hi2c2, MPU9250_ADDR, reg_addr, I2C_MEMADD_SIZE_8BIT, &val, 1, 100);
	}

	void MPU9250_Writebytes(uint8_t reg_addr, uint8_t len, uint8_t *data)
	{
		HAL_I2C_Mem_Write(&hi2c2, MPU9250_ADDR, reg_addr, I2C_MEMADD_SIZE_8BIT, data, len, 100);
	}

	void MPU9250_Readbyte(uint8_t reg_addr, uint8_t *data)
	{
		HAL_I2C_Mem_Read(&hi2c2, MPU9250_ADDR, reg_addr, I2C_MEMADD_SIZE_8BIT, data, 1, 100);
	}

	void MPU9250_Readbytes(uint8_t reg_addr, uint8_t len, uint8_t *data)
	{
		HAL_I2C_Mem_Read(&hi2c2, MPU9250_ADDR, reg_addr, I2C_MEMADD_SIZE_8BIT, data, len, 100);
	}
	/// @brief
	/// @param
	...
	void MPU9250_DataConvert(imu_9250_t *mpu9250)
	{
		...
		mpu9250->pt1_p.gyro_z = (mpu9250->pt1_p.gyro_z_raw) / (LSB_Sensitivity_GYRO * 10);
	}
